  ...
  return(
    <>
      <header className="sticky top-0 z-50"
        style={{background:"rgba(7,11,20,0.94)",backdropFilter:"blur(24px)",
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <motion.div initial={{opacity:0,x:-16}} animate={{opacity:1,x:0}} transition={{duration:0.5}}
            ...
            <div>
              <div className="font-display leading-none" style={{color:"white",fontSize:"clamp(15px,4vw,18px)"}}>
                Global<span style={{color:"var(--gold)",fontStyle:"italic"}}>Econ</span>
              </div>
          ...
          {/* Right: live pill + time + mobile menu btn */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={()=>setMenu(m=>!m)}
              ...
              style={{background: menu ? "rgba(245,200,66,0.1)" : "transparent"}}
              aria-label="Menu">
              {[0,1,2].map(i=>(
        ...
        {/* Desktop tab bar — scrollable on small screens */}
        <div className="max-w-screen-2xl mx-auto hidden lg:flex"
      ...
      <AnimatePresence>
        {menu&&(
          <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}
            className="fixed inset-x-0 top-[calc(14*4px)] z-40 shadow-2xl"
            style={{background:"rgba(11,15,26,0.98)",backdropFilter:"blur(24px)",
              borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
            <div className="max-w-screen-2xl mx-auto px-4 py-4 space-y-1">
              {TABS.map((tab,i)=>(
                <motion.button key={tab.id} initial={{opacity:0,x:-12}} animate={{opacity:1,x:0}}
                  transition={{delay:i*0.05}}
                  onClick={()=>handleTab(tab.id)}
                  className="w-full flex items-center gap-4 px-4 py-3.5 rounded-xl text-left transition-all"
                  style={{background:activeTab===tab.id ? `${tab.color}12` : "transparent",
                  ...
                  <span className="font-medium text-[14px]"
                    style={{color:activeTab===tab.id ? tab.color : "rgba(255,255,255,0.7)"}}>
                    {tab.label}
                  </span>
                  {activeTab===tab.id&&(
                    <span className="ml-auto font-mono text-[9px] px-2 py-0.5 rounded-full"
                      style={{background:`${tab.color}20`,color:tab.color}}>Active</span>
                  )}
                </motion.button>
              ))}
              {/* Mobile stats */}
